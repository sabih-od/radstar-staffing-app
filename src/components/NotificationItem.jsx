...
const NotificationItem = ({ item, navigation }) => {
    ...
    return (<TouchableOpacity
        ...
        style={[styles.notificationitem, {
            // borderLeftColor: item.isRead ? colors.primary : '#ddd',
        ...
        <View style={styles.notiInnerRow}>
            ...
            <View style={styles.notirowmsg}>
                ...
                <Text style={[styles.notimessage]} numberOfLines={2}>{item?.content ? item?.content : item?.message}</Text>
            </View>
            {/* </View> */}
            {/* <Text style={styles.datetime}>{moment(parseInt(item?.created_at)).format("DD MMM, YYYY hh:mm")}</Text> */}
        </View>
        <Text style={styles.notiTime}>{moment(parseInt(new Date().getTime())).format("hh:mm A")}</Text>
    </TouchableOpacity>)
}
...
const styles = StyleSheet.create({
    notificationitem: {
        ...
        // borderLeftWidth: 3,
        paddingHorizontal: isIPad ? 15 : 12, paddingVertical: isIPad ? 15 : 12, backgroundColor: colors.white, borderRadius: 15, marginBottom: 10
    },
    notiInnerRow: { flexDirection: 'row', alignItems: 'center', flex: 0.78 },
    image: { width: 60, height: 60, resizeMode: 'cover', borderRadius: 15, marginRight: 10 },
